    ...
    render() {
        ...
        if (!this.props.childrenLinks) {
            if (this.props.isHeader) {
                return (
                    <li className={[s.headerLink, this.props.className].join(' ')} style={{overflow: 'hidden'}}>
                        {this.props.isOutLink !== undefined ?
                            <a href={this.props.link} target='_blank' style={{height: 38}}>
              ...
              <span className={s.icon}>
                <i className={`fa ${this.props.iconName}`}/>
                            ...
                            <sup className={s.headerLabel}>{this.props.label}</sup>}
                                {this.props.badge && <Badge className={s.badge}>{this.props.badge}</Badge>}
                            </NavLink>
                        }
